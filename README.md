...
| `pop_back`   | 删除尾元素       | `void`     | `v.pop_back()`    |
| `push_front` | 头插          | `void`     | `v.push_front(1)` |
| `pop_front`  | 删除头元素       | `void`     | `v.pop_front()`   |
...
| `insert`     | 在迭代器前插入  | `iterator` | `l.insert(it,2)`  |
...
|                     | `empty/size` | 底层尺寸判断               | `bool`/`size_t` | `st.empty()`, `st.size()` |
| `queue<T>`          | `push`       | `con.push_back`      | `void`          | `q.push(2)`               |
|                     | `pop`        | `con.pop_front`      | `void`          | `q.pop()`                 |
|                     | `front/back` | `con.front/back`     | `T&`            | `q.front()`, `q.back()`   |
...
| 方法                         | 说明     | 返回值      | 示例                             |
| -------------------------- | ------ | -------- | ------------------------------ |
| `push(const T&)`           | BST 插入 | `bool`   | `bst.push(5)`                  |
| `pop(const T&)`            | 删除节点   | `bool`   | `bst.pop(5)`                   |
| `find(const T&)`           | 查找节点   | `Node*`  | `bst.find(3)`                  |
...
## 🌳 平衡 AVL 树 `Tree_Container::AVL_Tree<T>`
...
namespace Tree_Container 
{
    ...
    {
    private:
        struct Node{ T data; Node* left; Node* right; int height; };
...
| 方法                     | 说明      | 返回值      | 示例              |
| ---------------------- | ------- | -------- | --------------- |
| `push(const T&)`       | 插入并保持平衡 | `bool`   | `avl.push(5)`   |
| `pop(const T&)`        | 删除并保持平衡 | `bool`   | `avl.pop(5)`    |
| `find(const T&) const` | 查找节点    | `Node*`  | `avl.find(3)`   |
| `Middle_order_traversal()`      | 中序遍历输出  | `void`   | `avl.Middle_order_traversal()` |
| `Pre_order_traversal()`      | 前序遍历输出  | `void`   | `avl.Pre_order_traversal()` |
| `size() const`         | 返回节点数   | `size_t` | `avl.size()`    |
...
| `cbegin()`                 | 返回首元素常迭代器   | `const_iterator`         | `m.cbegin();`                   |
| `cend()`                   | 返回尾后常迭代器    | `const_iterator`         | `m.cend();`                     |
| `rbegin()`                 | 返回反向首元素迭代器  | `reverse_iterator`       | `m.rbegin();`                   |
| `rend()`                   | 返回反向尾后迭代器   | `reverse_iterator`       | `m.rend();`                     |
| `crbegin()`                | 返回反向首元素常迭代器 | `const_reverse_iterator` | `m.crbegin();`                  |
| `crend()`                  | 返回反向尾后常迭代器  | `const_reverse_iterator` | `m.crend();`                    |
| `operator[](Key_Val_Type)` | 访问或插入元素     | `iterator`               | `m[{1, "a"}]`                   |

对于 Set 容器，方法与 Map 基本相同，只是操作的是单一键类型而非键值对。
...
| 方法                | 说明        | 返回值              | 示例                         |
| ----------------- | --------- | ---------------- | -------------------------- |
| `push(key)`       | 插入键值对     | `bool`           | `umap.push({1, "a"})`      |
| `pop(key)`        | 删除键值对     | `bool`           | `umap.pop({1, "a"})`       |
| `find(key)`       | 查找键值对     | `iterator`       | `umap.find({1, "a"})`      |
| `size()`          | 返回元素个数    | `size_t`         | `umap.size()`              |
| `capacity()`      | 返回容量      | `size_t`         | `umap.capacity()`          |
| `empty()`         | 判断是否为空    | `bool`           | `umap.empty()`             |
| `begin()`         | 返回首元素迭代器  | `iterator`       | `umap.begin()`             |
| `end()`           | 返回尾后迭代器   | `iterator`       | `umap.end()`               |
| `cbegin()`        | 返回首元素常迭代器 | `const_iterator` | `umap.cbegin()`            |
| `cend()`          | 返回尾后常迭代器  | `const_iterator` | `umap.cend()`              |
| `operator[](key)` | 访问或插入元素   | `iterator`       | `auto it = umap[{1, "a"}]` |

**unordered\_set 方法**

| 方法                | 说明        | 返回值              | 示例                  |
| ----------------- | --------- | ---------------- | ------------------- |
| `push(key)`       | 插入元素      | `bool`           | `uset.push(1)`      |
| `pop(key)`        | 删除元素      | `bool`           | `uset.pop(1)`       |
| `find(key)`       | 查找元素      | `iterator`       | `uset.find(2)`      |
| `size()`          | 返回元素个数    | `size_t`         | `uset.size()`       |
| `capacity()`      | 返回容量      | `size_t`         | `uset.capacity()`   |
| `empty()`         | 判断是否为空    | `bool`           | `uset.empty()`      |
| `begin()`         | 返回首元素迭代器  | `iterator`       | `uset.begin()`      |
| `end()`           | 返回尾后迭代器   | `iterator`       | `uset.end()`        |
| `cbegin()`        | 返回首元素常迭代器 | `const_iterator` | `uset.cbegin()`     |
| `cend()`          | 返回尾后常迭代器  | `const_iterator` | `uset.cend()`       |
| `operator[](key)` | 访问元素      | `iterator`       | `auto it = uset[3]` |
...
---
## 🌳 基类容器 平衡红黑树 `Tree_Container::RB_Tree<T>`
...
| `Middle_order_traversal() const` | 中序遍历      | `void`   | `rbt.Middle_order_traversal()` |
| `size() const`                   | 返回节点数     | `size_t` | `rbt.size()`                   |

---

## 🗄️基类容器 哈希表 `Hash_table<Key,Value,HF>`
...
namespace bitset_Container 
{
    ...
    class Hash_Table 
    {
        ...
        MY_Template::vector_Container::vector<Node*> _Hash_Table; // 桶数组
        Type_imitation_function _Type_imitation_function; // 哈希函数
        Node* _previous_data = nullptr;      // 最后一个插入节点指针
        Node* _Head_data = nullptr;          // 第一个插入节点指针
    public:
...
| 方法                                 | 说明                | 返回值        | 示例                    |
| ---------------------------------- | ----------------- | ---------- | --------------------- |
| `push(const Hash_Table_Type_Val&)` | 插入并重哈希            | `bool`     | `ht.insert(1,100)`    |
| `pop(const Hash_Table_Type_Val&)`  | 删除键对应元素           | `bool`     | `ht.erase(1)`         |
| `find(const Hash_Table_Type_Val&)` | 返回元素指针或 `nullptr` | `iterator` | `auto v = ht.find(2)` |
